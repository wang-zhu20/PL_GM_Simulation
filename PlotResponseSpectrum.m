...
function [Period, PSA]=PlotResponseSpectrum(AccFileName) 
...
Period = logspace(-2, 1, 301);


%%%%%%%%%%%%%%%%% no need to change below %%%%%%%%%%%%%%%%%

%%% read in Strong Motion format
 file_in = fopen(AccFileName, 'r');
 for i=1:3  % change number of headerlines to skip
    ans = fgetl(file_in);
 end
 [NPTS ans] = fscanf(file_in, '%f', 1);
...
figure(3333)
subplot(2,1,1)
plot(time, acc); xlabel('time (s)'); ylabel('Sa (g)'); hold off
subplot(2,1,2)
loglog(Period, PSA, 'r'); hold on
xlabel('Period (sec)')
ylabel('PSA (g)')
